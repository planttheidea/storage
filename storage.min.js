...
else{if("session"===b)for(c in e.session)f.session(c)}else{for(c in e.local)f.local(c);for(c in e.session)f.session(c)}}var h=function(a){var b={}.toString,c=/^.*\s(\w+).*$/;return function(d){return d===a?"global":b.call(d).replace(c,"$1").toLowerCase()}}(this),n;try{g.localStorage.setItem("t","t"),g.localStorage.removeItem("t"),n=!0}catch(u){n=!1}var p=n,q;try{g.sessionStorage.setItem("t","t"),g.sessionStorage.removeItem("t"),q=!0}catch(v){q=!1}var r=q,e={local:function(){var a={};if(p)for(var b in g.localStorage){if(b&&
